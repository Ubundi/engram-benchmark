...
| P4 | Write an explicit related-work matrix against LongMemEval and other memory benchmarks | Done | P0 | Research | `docs/related_work.md` now compares Engram against LongMemEval, LoCoMo, MemBench, and MemoryAgentBench | Repo-level table comparing task design, runtime setup, memory horizon, scoring, and artifact availability |
| P5 | Expand the evaluated system set beyond baseline vs Cortex | In progress | P0 | Research + Eng | Results still center on OpenClaw native vs Cortex, but `docs/system_matrix.md` now defines the current OpenClaw lineup as baseline plus the Mem0, ClawVault, and Cortex memory conditions | Main benchmark table with at least 3-5 systems or conditions and reproducible run artifacts |
| P6 | Repeat benchmark runs and report variance | In progress | P0 | Research + Eng | Current docs still reflect too few runs, but `docs/repeated_runs.md` now provides a fill-in template for per-run logs, aggregate variance, and paper tables | Reported means plus variance or confidence intervals for each main condition |
| P7 | Run a judge validation study against human labels | In progress | P0 | Research | No study results yet, but `docs/judge_validation.md` now provides a fill-in template for sample design, annotation protocol, agreement metrics, and appendix reporting | Annotation protocol, sample set, inter-annotator agreement, and judge-human agreement in paper appendix and repo docs |
| P8 | Strengthen external validity beyond one synthetic project framing | In progress | P0 | Research | Dataset card still notes synthetic, single-project limitations, but `docs/external_validity.md` now provides a fill-in template for either a second-corpus path or a synthetic stress-test defense | Either a second domain/corpus or a clearly defended benchmark-design section showing why synthetic stress tests are appropriate |
| P9 | Formalize benchmark governance and versioning policy | In progress | P1 | Benchmark Maintainer | Release and leaderboard docs exist, and `docs/governance.md` now provides a fill-in template for change classes, comparability, submission status, and historical result policy | Public versioning doc covering benchmark changes, leaderboard eligibility, and historical comparability |
