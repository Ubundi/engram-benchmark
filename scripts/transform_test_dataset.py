...
def transform_user_turn(content: str) -> str:
    ...
    if re.match(r"\*?\*?\[TASK ASSIGNMENT", content, re.IGNORECASE):
        ...
        reframed = re.sub(
            r"Worker, we need you to execute", "Worker, you were asked to execute", reframed
        )
...
if __name__ == "__main__":
    ...
    if "--dry-run" in sys.argv:
        ...
        action_patterns = [
            ...
            r"\[TASK ASSIGNMENT",
            r"\[HEARTBEAT TRIGGER",
            r"(do it|go ahead|lock it|wrap it|close out)",
        ]
                    ...
                    for pat in action_patterns:
                        if _re.search(pat, turn["content"], _re.IGNORECASE):
                            ...
                            if len(still_examples) < 15:
                                still_examples.append(
                                    f"  {item['question_id']} S{si}T{ti}: {turn['content'][:100].replace(chr(10), ' ')}"
                                )
                            break
